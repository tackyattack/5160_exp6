...
uint8_t Read_Sector(uint32_t sector_number, uint16_t sector_size, uint8_t *data_array)
{
  ...
  if(error_flag == no_errors) read_block_i(sector_size, data_array);
...
uint8_t Read_Sector_i(uint32_t sector_number, uint16_t sector_size, uint8_t *data_array)
{
  uint8_t SD_type, error_flag;
  SD_type = Return_SD_Card_Type();
  SPI_Select_Clear(SD_Card_Select);
  error_flag = SEND_COMMAND_i(17, (sector_number<<SD_type));
...
// Reads single byte
uint8_t read8_i(uint16_t offset, uint8_t *data_array)
{
  offset = offset & 0x1FF; // mask off for safety
  return data_array[offset];
}
...
// Reads little endian 16b value
uint16_t read16_i(uint16_t offset, uint8_t *data_array)
{
  uint16_t ret_val;
  offset = offset & 0x1FF; // mask off for safety
  ret_val = 0;
...
// Reads little endian 32b value
uint32_t read32_i(uint16_t offset, uint8_t *data_array)
{
  uint32_t ret_val;
  uint8_t temp, i;
  offset = offset & 0x1FF; // mask off for safety
