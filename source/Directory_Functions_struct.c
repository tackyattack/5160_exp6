...
uint16_t get_entry(uint32_t Sector_num, uint16_t entry_num, entry_t *entry ,uint8_t xdata * array_in)
 {
   ...
   entry_num = 32*entry_num;
   Sector=Sector_num + entry_num/510;
   entry_num = entry_num%510;
   if(Sector-Sector_num>max_sectors)
   {
   ...
   if(error_flag==no_errors)
   {
     for(j=0; j<12; j++) entry->name[j] = '\0';
     temp8=read8(0+entry_num,values);  // read first byte to see if empty
     if((temp8!=0xE5)&&(temp8!=0x00))
     {
       attr=read8(0x0b+entry_num,values);
       entry->entry_attr = attr;

       // get the cluster
       // ----
       if(Drive_values.FATtype==FAT32)
       {
         entry->cluster=read8(21+entry_num,values);
         entry->cluster=entry->cluster<<8;
         entry->cluster|=read8(20+entry_num,values);
         entry->cluster=entry->cluster<<8;
       }
       entry->cluster|=read8(27+entry_num,values);
       entry->cluster=entry->cluster<<8;
       entry->cluster|=read8(26+entry_num,values);
       if(attr&0x10) entry->cluster |= directory_bit;
       // ----
       ...
       if((attr&0x0E)==0)   // if hidden, system or Vol_ID bit is set do not print
       {
         for(j=0;j<8;j++)
         {
           out_val=read8(entry_num+j,values);   // print the 8 byte name
           //putchar(out_val);
           entry->name[j] = out_val;
         }
         if((attr&0x10)==0x10)  // indicates directory
         {
           for(j=8;j<11;j++)
           {
             out_val=read8(entry_num+j,values);
             //putchar(out_val);
             entry->name[j] = out_val;
           }
           //printf("[DIR]\n");
     ...
     if(temp8 == 0) return NO_MORE_ENTRIES;
   }
   else
   {
     return NO_MORE_ENTRIES;
