...
uint32_t idata data_pool32[4];
uint16_t idata data_pool16[3];
uint8_t  idata data_pool8[4];
...
//       set_data_pool_dir(data_pool32, data_pool16, data_pool8);
//       set_data_pool_player(data_pool32, data_pool16, data_pool8);
...
//     case print_buffer:
//       LED_number(3);
//       UART_Transmit(get_directory_print_buffer()[temp16]);
//       temp16++;
...
void run_read_dir_entry_task_task()
{
  LED_number(5);
  if(read_dir_entry_task(&clus) == TASK_RUNNING)
  {
...
void sEOS_ISR(void) interrupt 2 using 1
{
